...
modalTrigger.forEach(btn => {
 btn.addEventListener('click', () => openModal(modalSelector, modalTimerId)); //обернули фун-ию в стрелочную ф-ию, чтобы вызвалась только по клику
});
...
/* harmony export */   "default": () => (__WEBPACK_DEFAULT_EXPORT__)
/* harmony export */ });
function slider({countainer, slide, nextArrow, prevArrow, totalCounter, currentCounter, wrapper, field}) {
            //slider
        ...
        if (offset == cutPx(width)* (slides.length -1)) { //width =650px - строка. Преобразовали в число
            offset = 0;
        } else { offset += cutPx(width);
        ...
        prev.addEventListener('click', () => {
            if (offset == 0) { 
                offset = cutPx(width) * (slides.length -1);
            } else { offset -= cutPx(width);
...
function tabs(tabsSlector, tabsContentSelector, tabsParentSelector, activeClass) {
	...
	let tabs = document.querySelectorAll(tabsSlector),
